...
- `linux-headers`: sanitized userspace UAPI headers generated by the same board
  kernel's `make headers` target

The development package has an exact-version dependency on the generated
`linux-headers` package. The rootfs installs both the custom kernel and its
